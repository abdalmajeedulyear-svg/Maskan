...
public class MainActivity extends AppCompatActivity {
    ...
    private void updateRecyclerView(List<Property> properties) {
        ...
        PropertyAdapter featuredAdapter = new PropertyAdapter(properties.subList(0, Math.min(3, properties.size())));
        rvFeatured.setAdapter(featuredAdapter);

        // تحديث العقارات القريبة (أول 6 عقارات)
        PropertyAdapter nearbyAdapter = new PropertyAdapter(properties.subList(0, Math.min(6, properties.size())));
        rvNearby.setAdapter(nearbyAdapter);
